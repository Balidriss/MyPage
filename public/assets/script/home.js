...
class Quiz {
  ...
  constructor(index) {
    ...
    if (!this.element) {
      throw new Error(
        `quiz element failed to be assign to instance guess #${index} with expected selector : ${
          Quiz.baseSelector + index
        }`
      );
    }
  ...
  static assignSliderEvents() {
    ...
    Quiz.quizContainer.addEventListener('mousedown', Quiz.dragStart, {
      passive: false,
    });
    Quiz.quizContainer.addEventListener('mousemove', Quiz.dragAction, {
      passive: false,
    });
  ...
  static update() {
    ...
    Quiz.numberToShow = Math.floor(
      Quiz.quizContainer.offsetWidth / Quiz.quiz[1].element.offsetWidth ?? 1
    );
    for (let index = 0; index < Quiz.quiz.length; index++) {
      ...
      Quiz.quiz[index].element.style.right =
        Quiz.frontPos + Quiz.gap * index - Quiz.gap + '%';
      ...
      Quiz.quiz[index].element.style.backgroundColor = `rgb(${
        (Quiz.numberToShow - index) * 10
      },${(Quiz.numberToShow - index) * 10},${
        (Quiz.numberToShow - index) * 10
      }`;
      zIndex--;
    ...
    Quiz.applyText(
      Quiz.quiz[1].answer,
      Quiz.quiz[1].element.querySelector('.answer')
    );
  }
    ...
    fetch('/attempt', {
      ...
      .then((data) => {
        ...
        if (
          ...
          Quiz.applyText(
            Quiz.quiz[1].successMessage,
            this.querySelector('.success-message')
          );
          Quiz.update();
        }
      })
...
class CV {
  ...
  static init() {
    CV.pairTabContents = [
      new CVPart(
        document.querySelector('.tabs-cv .formations'),
        document.getElementById('formations')
      ),
      new CVPart(
        document.querySelector('.tabs-cv .professions'),
        document.getElementById('professions')
      ),
      new CVPart(
        document.querySelector('.tabs-cv .projects'),
        document.getElementById('projects')
      ),
    ];

    document.querySelector('.tabs-cv').setAttribute('role', 'tablist');
    ...
    CV.pairTabContents.forEach((part, index) => {
      ...
      part.tab.addEventListener('keydown', (event) =>
        CV.handleKeyDown(event, index)
      );
      part.tab.addEventListener('keydown', (event) =>
        CV.handlePanelScroll(event, part.content)
      );
    });
  ...
  static handleKeyDown(event, index) {
    ...
    switch (event.key) {
      ...
      case 'ArrowLeft':
        newIndex =
          (index - 1 + CV.pairTabContents.length) % CV.pairTabContents.length;
        break;
      case 'Home':
        newIndex = 0;
        break;
